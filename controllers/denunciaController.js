...
exports.crearDenuncia = async (req, res) => {
    try {
        ...
        // Si hay archivos adjuntos, guardarlos en la colección de Evidencia
        let archivosGuardados = 0;
        if (req.files && req.files.length > 0) {
            console.log(`📁 Procesando ${req.files.length} archivo(s) para denuncia ${denuncia.id}`);
            for (const file of req.files) {
                try {
                    console.log(`Guardando archivo: ${file.originalname}`);
                    await Evidencia.crear({
            ...
            console.log(`📊 Total archivos guardados: ${archivosGuardados}/${req.files.length}`);
        } else {
            console.log('ℹ️ Sin archivos adjuntos');
        }
        ...
        res.status(201).json({
            ...
            denuncia,
            archivos_subidos: archivosGuardados || 0
        });
...
exports.obtenerArchivos = async (req, res) => {
    try {
        ...
        const archivos = await Evidencia.obtenerPorDenuncia(id);
